...
export default function GatePassScreen({navigation}){
    ...
    const sendNotification = async () => {
        const message = {
            ...
            body: gateResponse.response_message
        }
    ...
    const getGateRates = () => {
        fetch(`${CENTRAL_API}agent/concessionaires-product-codes?category=GatePass`, {
            headers:{
                'Accept': 'application/json',
                'content-type': 'application/json',
            },
    ...
    const processGatePass = () =>{
        if(tonnage === null || plateNumber === '' || name === '' || phone === '' || collectionPoint === '') {
                                ...
                                <View style={{marginTop: 10, marginLeft: 16}}>
                                    {
                                        gateResponse.response_code === '00' || gateResponse.response_code === '01' ? (
                                            ...
                                            <View>
                                                ...
                                                <Text style={{fontWeight: '700', textAlign: 'center'}}>{gateResponse.response_message || gateResponse.message}</Text>
                                                <Button
                                                    title="Try Again"
                                                    titleStyle={styles.btnText}
                                                    onPress={() => hideModal()}
