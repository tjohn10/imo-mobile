...
export default function ForestryScreen({navigation}){
    ...
    const [forestryResponse, setForestryResponse] = useState([])
    ...
    const sendNotification = async () => {
        const message = {
            to: expoPushToken,
            sound: 'default',
            title: 'Forestry Ticket' + ' - ' + plateNumber,
            body: forestryResponse.response_message
        }
    ...
    const getVehicleType = () => {
        fetch(`${CENTRAL_API}agent/concessionaires-product-codes?category=Forestry`, {
            headers:{
                'accept': 'application/json',
                'content-type': 'application/json',
            },
    ...
    const onSubmit = () =>{
        if(content === '' || tonnage === null || plateNumber === '' || name === '' || phone === '' || collectionPoint === '') {
            ...
            fetch(`${MOBILE_API}transport/create-haulage`, {
                ...
                .then((responseJson) => {
                    setLoading(false)
                    setForestryResponse(responseJson)
                    sendNotification()
                })
                .catch((e) => {
                    console.log(e)
                    ...
                    <Modal visible={visible} onDismiss={hideModal} contentContainerStyle={containerStyle}>
                        {
                            loading ? <ActivityIndicator color="#09893E" size="large"/> : (
                                <View style={{marginTop: 10, marginLeft: 16}}>
                                    {
                                        forestryResponse.response_code === '00' || forestryResponse.response_code === '01' ? (
                                            <View>
                                                <Image style={{
                                                    width: 100,
                                                    height: 100,
                                                ...
                                                <Text style={{fontWeight: '700', textAlign: 'center'}}>{forestryResponse.response_message || 'Payment Successful'}</Text>
                                                <Text style={{fontWeight: '700', textAlign: 'center'}}>REF: {forestryResponse.payment_ref}</Text>
                                               <Button
                                                    title="Show Receipt"
                                                    titleStyle={styles.btnText}
                                                    onPress={() => navigation.navigate("Done", {params: forestryResponse})}
                                            ...
                                            <View>
                                                ...
                                                <Text style={{fontWeight: '700', textAlign: 'center'}}>{forestryResponse.response_message || forestryResponse.message}</Text>
                                                <Button
                                                    title="Try Again"
                                                    titleStyle={styles.btnText}
                                                    onPress={() => hideModal()}
