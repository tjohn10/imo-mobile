...
export default function MarketDailyScreen({navigation}){
    ...
    const sendNotification = async () => {
        const message = {
            ...
            body: marketResponse.response_message
        }
    ...
    const getMarketTolls = () => {
        fetch(`${CENTRAL_API}agent/concessionaires-product-codes?category=Market`, {
            headers: {
                'accept': 'application/json',
                'Content-Type': 'application/json'
            },
            ...
            .then((resJson) => {
                setCollectionType(resJson)
            })
    }
    const getIncomeAmount = (selected) => {
    ...
    const processMarketTicket = () =>{
        if(content === '' || collection === null || plateNumber === '' || name === '' || phone === '' || collectionPoint === '') {
            ...
            ConcessionStore.update((s) => {
                ...
                s.tonnage = collection.productCode
            ...
            fetch(`${MOBILE_API}transport/create-haulage`, {
                ...
                body: JSON.stringify({
                    ...
                    "total_number": 1,
                    "vehicle_content": "Market",
                    "product_code": collection.productCode,
                    "category": "Market",
                    "taxPayerPhone": phone,
                    "taxPayerName": name,
                    "plateNumber": " ",
                ...
                .then((responseJson) => {
                    setLoading(false)
                    setMarketResponse(responseJson)
                    ...
                    <Modal visible={visible} onDismiss={hideModal} contentContainerStyle={containerStyle}>
                        {
                            loading ? <ActivityIndicator color="#09893E" size="large"/> : (
                                <View style={{marginTop: 10, marginLeft: 16}}>
                                    {
                                        marketResponse.response_code === '00' || marketResponse.response_code === '01' ? (
                                            <View>
                                                <Image style={{
                                                    width: 100,
                                                    height: 100,
                                                ...
                                                <Text style={{fontWeight: '700', textAlign: 'center'}}>{marketResponse.response_message || 'Payment Successful'}</Text>
                                                <Text style={{fontWeight: '700', textAlign: 'center'}}>REF: {marketResponse.payment_ref}</Text>
                                                <Button
                                                    title="Show Receipt"
                                                    titleStyle={styles.btnText}
                                                    onPress={() => navigation.navigate("Done", {params: marketResponse})}
                                            ...
                                            <View>
                                                ...
                                                <Text style={{fontWeight: '700', textAlign: 'center'}}>{marketResponse.response_message || marketResponse.message}</Text>
                                                <Button
                                                    title="Try Again"
                                                    titleStyle={styles.btnText}
                                                    onPress={() => hideModal()}
                ...
                <View style={{marginTop: 5}}>
                    ...
                    {/*<View style={{flexDirection: 'row', justifyContent: 'space-between'}}>*/}
                        <Picker
                            ...
                            <Picker.Item label="Collection Type" value="" />
                            {
